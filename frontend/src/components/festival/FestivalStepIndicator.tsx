...
import { Sparkles, Heart, Bell, Calendar, CheckCircle2, Flame } from 'lucide-react';
...
export const FestivalStepIndicator: React.FC<FestivalStepIndicatorProps> = ({
  ...
  const steps = [
    ...
    { num: 2, label: 'Darshan', icon: Flame },
    { num: 3, label: 'Notifications', icon: Bell },
    { num: 4, label: 'Updates', icon: Calendar },
    { num: 5, label: 'Donate', icon: Heart },
    { num: 6, label: 'Receipt', icon: CheckCircle2 },
  ];
    ...
    <div className="w-full max-w-4xl mx-auto px-4 py-4">
      ...
      <div className="flex items-center justify-between sm:hidden bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl p-3 shadow-lg">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <div className="text-[10px] uppercase font-bold text-orange-400 tracking-wider">Step {currentStep} of 6</div>
      ...
      <div className="hidden sm:flex items-center justify-between relative bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-2xl p-2.5 shadow-xl">
        {steps.map((s, idx) => {
          const Icon = s.icon;
